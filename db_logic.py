...
def get_task_by_title(title, user_id):
    return Task.query.filter(Task.title == title).filter(Task.user_id == user_id).first()

def get_tasks_by_user_id(id):
    return [task for task in Task.query.filter(Task.user_id == id).order_by(Task.id).all()]
